...
#====================================================
#	System Request:Debian 9+10/Ubuntu 18+20/Centos 7+
#	Author:	Dexter Eskalarte
#	Dscription: V2ray ws+tls onekey Management
#	Version: 1.0
#	email:admin@mtkdeveloper.com
...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/${github_branch}/tls/v2ray.sh
...
domain_check() {
    ...
    if [[ ${wgcfv4_status} =~ "on"|"plus" ]] || [[ ${wgcfv6_status} =~ "on"|"plus" ]]; then
        # Turn off wgcf-warp to prevent misjudgment of the VPS IP situation
    ...
    if [[ -z ${local_ipv4} && -n ${local_ipv6} ]]; then
        ...
        echo -e "${OK} ${GreenBG} identified as IPv6 Only VPS, automatically added DNS64 server ${Font}"
    fi
    echo -e "IP resolved by domain name DNS: ${domain_ip}"
    ...
    if [[ ${domain_ip} == ${local_ipv4} ]]; then
        ...
        echo -e "${Error} ${RedBG} Please ensure that the correct A/AAAA record is added to the domain name, otherwise V2ray ${Font} will not work properly"
        echo -e "${Error} ${RedBG} domain name DNS resolution IP does not match the local IPv4 / IPv6. Do you want to continue the installation? (y/n) ${Font}" && read -r install
        case $install in
        [yY][eE][sS] | [yY])
            echo -e "${GreenBG} continue installing ${Font}"
            sleep 2
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_inbound_port
    modify_UUID
}
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/${github_branch}/http2/config.json -O config.json
    modify_path
    modify_inbound_port
    modify_UUID
}
...
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/${github_branch}/tls/ssl_update.sh"
...
bbr_boost_sh() {
    ...
    wget -N --no-check-certificate "https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/${github_branch}/tls/tcp.sh" && chmod +x tcp.sh && ./tcp.sh
}
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/${github_branch}/tls/install.sh | grep "shell_version=" | head -1 | awk -F '=| "' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/${github_branch}/tls/install.sh
...
echo -e " \033[0;35m══════════════════════════════════════════════════════════════════\033[0m"
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/${github_branch}/tls/v2ray.sh)
        ;;
    4)
    ...
    17)
        ...
        ;;
    18)
        read -rp "Please enter the camouflage path (note! No need to add a slash eg:ray):" camouflage_path
        modify_camouflage_path
        start_process_systemd
        ;;
    *)
