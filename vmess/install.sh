...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/server_script/tls/v2ay.sh
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/server_script/tls/config.json -O config.json
    modify_path
    modify_inbound_port
    modify_UUID
}
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/server_script/http2/config.json -O config.json
    modify_path
    modify_inbound_port
    modify_UUID
}
...
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/server_script/tls/ssl_update.sh"
...
bbr_boost_sh() {
    ...
    wget -N --no-check-certificate "https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/server_script/tls/tcp.sh" && chmod +x tcp.sh && ./tcp.sh
}
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/server_script/tls/install.sh | grep "shell_version=" | head -1 | awk -F '=| "' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/Dexter-Hysteria/v2ray/server_script/tls/install.sh
